...
function InfoRow({ icon, primary, secondary }) {
  return (
    <div className="flex items-center gap-3">
      <div className="w-10 h-10 bg-zinc-900 border border-zinc-800 rounded-xl flex items-center justify-center text-zinc-400 flex-shrink-0">
        {icon}
      </div>
      <div>
    ...
    <>
      ...
      <div className="min-h-screen bg-[#09090B] anim-page">
        <div className="relative h-[46vh] overflow-hidden">
          ...
          <Link href="/discover" className="absolute top-14 left-4 w-10 h-10 bg-black/50 backdrop-blur-xl rounded-full flex items-center justify-center border border-white/10 text-white pressable">
            <BackIcon />
          </Link>
        ...
        <div className="px-4 pb-36 -mt-2">
          ...
          <h1 className="text-[28px] font-extrabold text-white leading-tight tracking-tight mb-6">{event.title}</h1>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50">
          <div className="max-w-lg mx-auto px-4 pb-8 pt-4 bg-gradient-to-t from-[#09090B] via-[#09090B]/95 to-transparent">
            {owned ? (
              <Link href={'/ticket/' + event.id} className="w-full py-4 rounded-2xl font-bold text-[16px] tracking-wide flex items-center justify-center bg-emerald-600 text-white shadow-lg shadow-emerald-900/30 pressable">
                View My Ticket
              </Link>
            ) : isSoldOut || event.soldOut ? (
              <div className="w-full py-4 rounded-2xl font-bold text-[16px] tracking-wide flex items-center justify-center bg-zinc-800 text-zinc-500 border border-zinc-700 cursor-not-allowed">
                Sold Out
              </div>
            ) : event.salesPaused ? (
              <div className="w-full py-4 rounded-2xl font-bold text-[16px] tracking-wide flex items-center justify-center bg-zinc-800 text-amber-500/70 border border-zinc-700 cursor-not-allowed">
                Sales Paused
              </div>
            ) : (
              <button onClick={handleGetTicket} disabled={loading} className="w-full py-4 rounded-2xl font-bold text-[16px] tracking-wide flex items-center justify-center gap-2 bg-red-600 text-white shadow-lg shadow-red-900/40 disabled:opacity-60 pressable">
                {loading ? (
