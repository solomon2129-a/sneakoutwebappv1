...
function SettingRow({ icon, label, sublabel, danger = false, onClick, href }) {
  const inner = (
    <div className={"flex items-center gap-3 py-4 border-b border-zinc-800/60 last:border-0 pressable" + (danger ? ' text-rose-500' : '')}>
      <div className={"w-9 h-9 rounded-xl flex items-center justify-center" + (danger ? ' bg-rose-900/30 text-rose-400' : ' bg-zinc-800 text-zinc-400')}>
        {icon}
      </div>
      <div className="flex-1">
        <p className={"text-sm font-medium" + (danger ? ' text-rose-500' : ' text-zinc-200')}>{label}</p>
