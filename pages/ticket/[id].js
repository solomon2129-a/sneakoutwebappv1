...
function CheckCircleIcon() {
  return (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>);
}
    ...
    <>
      ...
      <div className="min-h-screen bg-[#09090B] flex flex-col anim-page">
        <div className="flex items-center justify-between px-4 pt-14 pb-4">
          <Link href="/discover" className="w-10 h-10 bg-zinc-900 border border-zinc-800 rounded-full flex items-center justify-center text-zinc-400 pressable">
            <BackIcon />
          </Link>
          <h1 className="text-[16px] font-bold text-white">Your Ticket</h1>
          <button onClick={handleShare} className="w-10 h-10 bg-zinc-900 border border-zinc-800 rounded-full flex items-center justify-center text-zinc-400 pressable">
            <ShareIcon />
          </button>
        </div>
        ...
        <div className="flex-1 flex items-start justify-center px-4 pb-12 pt-2">
          <div className="w-full max-w-sm anim-fade-up">
            <div className="ticket-body">
              <div className="relative h-44 overflow-hidden">
                ...
                <div className="absolute top-4 left-4 flex items-center gap-1.5">
                  <div className="w-6 h-6 rounded-md bg-red-600 flex items-center justify-center text-white text-[11px] font-black">S</div>
                  <span className="text-white font-bold text-[13px] tracking-wide">SNEAKOUT</span>
                </div>
                ...
                <div className="absolute bottom-4 left-4 right-4">
                  <h2 className="text-white text-xl font-black leading-tight tracking-tight">{ticket.title}</h2>
                </div>
              </div>
              ...
              <div className="flex flex-col items-center px-6 pb-6">
                <div className="bg-white rounded-2xl p-4 mb-5 shadow-xl anim-fade-pop">
                  <QRCodeSVG value={qrValue} size={160} bgColor="#FFFFFF" fgColor="#09090B" level="H" includeMargin={false} />
                </div>
            ...
            <div className="flex gap-3 mt-4">
              <Link
                href={'/e/' + ticket.id}
                className="flex-1 py-3.5 rounded-2xl bg-zinc-900 border border-zinc-800 text-zinc-300 text-sm font-semibold flex items-center justify-center pressable"
              >
                Event Details
              </Link>
              <button
                onClick={handleShare}
                className="flex-1 py-3.5 rounded-2xl bg-red-600 text-white text-sm font-semibold flex items-center justify-center gap-2 pressable"
              >
                <ShareIcon /> Share
              </button>
            </div>
